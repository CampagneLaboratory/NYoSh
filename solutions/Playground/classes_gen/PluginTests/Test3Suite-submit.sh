...
job_arguments="--plugins-dir ${PLUGINS_DIR} --job GOBYWEB_PLUGIN_TEST_RUNNER --job-tag CHZQJJM --owner gobyweb --queue rascals.q --job-area gobyweb@darla.med.cornell.edu:/zenodotus/campagnelab/scratch/data/gobyweb/trial/GOBYWEB_SGE_JOBS/ --fileset-area ${FILESET_AREA} --repository /scratchLocal/gobyweb/ARTIFACT_REPOSITORY-NYoSh/ --artifact-server gobyweb@darla.med.cornell.edu:/zenodotus/campagnelab/scratch/data/gobyweb/test/gobyweb2-plugins --broker-hostname toulouse.qib.pbtech --broker-port 5672 --TEST_NAMES Test3Suite "
clean_tag gobyweb@darla.med.cornell.edu:/zenodotus/campagnelab/scratch/data/gobyweb/trial/GOBYWEB_SGE_JOBS/ gobyweb C/CHZQJJM
plugins-submit-job ${job_arguments} --depend-on TKSFBCZ TEST_CLASSES: ${CLASSES_TAG} ${COPY_BACK_LOCATION_OPTION}
