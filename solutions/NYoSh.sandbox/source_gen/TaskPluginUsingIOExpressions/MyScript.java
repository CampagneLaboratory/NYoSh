package TaskPluginUsingIOExpressions;
      ...
      try {
        {
          ...
          for (ScriptVariable var : variables_t90jbv_a0a0a0a5a0a0a0g) {
          ...
          for (ScriptVariable var : variables_t90jbv_b0a0a0f0a0a0a6) {
            exportedVariables.add(var.name);
          }

          if (check(NYoShRuntimeEnvironment.getEnvironment().getVariableValue("FILESET_COMMAND"), "TEXT")) {
            String inputTXTFiles = fetch(NYoShRuntimeEnvironment.getEnvironment().getVariableValue("FILESET_COMMAND"), "TEXT");
            System.out.println(String.format("Input text files: %s", inputTXTFiles));
            // process the input files...      
          } else {
            fail(false, "Input text files are not available", -1);
          }

          {
            StringBuffer commandBuffer = new StringBuffer();
            CommandAssembler assembler = new CommandAssembler();
            assembler.appendBashFragment("echo output-string1 > "+NYoShRuntimeEnvironment.getEnvironment().getVariableValue("JOB_DIR")+"/output-stats-1.tsv");
            //  process output according to type 
            CommandExecutionPlan plan = null;
            lastExitCode = -1;
            try {
          ...
          if (lastExitCode == 0) {
            System.out.println("TSV 1 successfully generated");
          } else {
            fail(false, "Failed to generate TSV 1", 127);
          }
          {
            StringBuffer commandBuffer = new StringBuffer();
            CommandAssembler assembler = new CommandAssembler();
            assembler.appendBashFragment("echo output-string2 > "+NYoShRuntimeEnvironment.getEnvironment().getVariableValue("JOB_DIR")+"/output-stats-2.tsv");
            //  process output according to type 
            CommandExecutionPlan plan = null;
            lastExitCode = -1;
            try {
          ...
          if (lastExitCode == 0) {
            System.out.println("TSV 2 successfully generated");
          } else {
            fail(false, "Failed to generate TSV 2", 127);
          }

          // publish results... 
          String outputfiles = new PathPatternMatcher().match(NYoShRuntimeEnvironment.getEnvironment().getVariableValue("JOB_DIR")+"/*.tsv").exclude().resolve();
          System.out.println(String.format("Pushing results files: %s", outputfiles));
          String tags = push(NYoShRuntimeEnvironment.getEnvironment().getVariableValue("FILESET_COMMAND"), "STATS", outputfiles);
          System.out.println(String.format("STATS instances successfully pushed. Assigned tags: %s", tags));
        }
